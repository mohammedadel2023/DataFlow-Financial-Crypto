...
def write_on_minio(docs: list, connect_str:str) -> None:
	setting = get_setting()
	...
	for doc in docs:
		...
		try :
			...
			update_status(doc["list_of_art"], connect_str)
	...
	with psycopg.connect(connect_str) as conn:
		try :
			...
			with conn.cursor() as cur:
				...
				for doc in docs:
					for art in doc["list_of_art"]:
						if art["write_on_postgress"] == False:
							continue
						cur.execute(f"""
					 INSERT INTO {table} (content_hash, title, publish_date, scraped_at, status)
					  VALUES (%s, %s, %s, %s, %s)
					 """,(art["hash"], art["art_title"], art["time"], now,'uploaded'))
					written_art += 1
			conn.commit()
...
def update_status(updating_list: list, connect_str:str, table:str ="batch_data") -> None:
	logger.debug("establish a connection with postgreSQL for updating status")
	with psycopg.connect(connect_str) as conn:
		try :
			logger.debug("the connection with postgreSQL is established successfuly for updating status")
			with conn.cursor() as cur:

				for art in updating_list:

					cur.execute(f"""
					 UPDATE {table} SET status = 'pending' WHERE content_hash = %s
					 """,(art["hash"],))
				conn.commit()
